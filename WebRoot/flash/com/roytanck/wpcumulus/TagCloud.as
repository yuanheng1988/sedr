...
package com.roytanck.wpcumulus
{
	...
	public class TagCloud extends MovieClip	{
		...
		public function TagCloud(){
			...
			var item:ContextMenuItem = new ContextMenuItem("TagCloud中文版 by Jessica");
			...
			if( this.loaderInfo.parameters.mode == null )	{
				...
				// load XML file
				var myXMLURL:URLRequest = new URLRequest( baseURL + "tagcloud.xml" );
		...
		private function init( o:XML ):void {
			...
			for each( var node2:XML in o.a ){
				...
				// create mc
				var mc:Tag = new Tag( node2, getColorFromGradient( perc ), ( hicolor == tcolor ) ? getColorFromGradient( perc ) : hicolor );
				holder.addChild(mc);
				// store reference
				mcList.push( mc );
			}
		...
		private function menuItemSelectHandler( e:ContextMenuEvent ):void {
			var request:URLRequest = new URLRequest( "http://www.skiyo.cn" );
